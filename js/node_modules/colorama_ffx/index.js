...
exports.writeColors = function(colors, positions, opacities) {
    if (fs.existsSync(out_path))
        fs.unlinkSync(out_path);

    let output = Array.from(template);
    colors.forEach((e, i) => {
        let opacity = Math.round(opacities[i] * 255).toString(16).toUpperCase();
        let out_str = (positions[i] * 0xFFFF).toString(16).split('.')[0].padStart(8, '0') + opacity + e;
